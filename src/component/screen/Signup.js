...
class SignUp extends Component {
  ...
  onSignUpPress = () => {
    ...
    if (this.state.name.length < 2) {
      this.setState({
        nameErrorMessage: 'Name length should be more than 2',
        nameError: true,
        nameColor: '#d61818',
      });
    } else {
    ...
    if (validationRegex.password.test(this.state.password) === false) {
      this.setState({
        passwordErrorMessage:
          'Password length should be more than 5 with minimum one letter, one number and one special character',
        passwordError: true,
