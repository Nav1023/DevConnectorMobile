...
class Login extends Component {
  ...
  onLoginPress = () => {
    ...
    if (this.state.password.length < 6) {
      this.setState({ passwordErrorMessage: 'Password length should be more than 5', passwordError: true, passwordColor: '#d61818' });
    } else {
  ...
  onChangeTextPassword = (text) => {
    this.setState({ password: text, passwordErrorMessage: '', passwordError: false });
  }
        ...
        >
          ...
          <View style={{ marginTop: verticalScale(70), marginHorizontal: horizontalScale(20) }}>
            ...
            <MyButton
              ...
              style={[styles.button, { shadowColor: '#515a5a', backgroundColor: '#fff8e5',  marginTop: verticalScale(30)}]}
              textStyle={{ color: 'black' }}
            />
          </View>
          <View style={{marginTop: verticalScale(5), justifyContent: 'center', flexDirection: 'row'}}>
                <Text style={[styles.text, { fontSize:12}]}>Don't have account ? </Text>
                <Text 
